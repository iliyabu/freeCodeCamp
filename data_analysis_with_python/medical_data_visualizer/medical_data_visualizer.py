...
def draw_cat_plot():
    ...
    fig = sns.catplot(data=df_cat, kind='count', x='variable', hue='value', col='cardio').set_ylabels('total').figure
...
def draw_heat_map():
    ...
    # height is less than the 2.5th percentile and height is more than the 97.5th percentile
    df_heat = df_heat[df['height'] >= df['height'].quantile(0.025)]
    df_heat = df_heat[df['height'] <= df['height'].quantile(0.975)]

    # weight is less than the 2.5th percentile and weight is more than the 97.5th percentile
    ...
    sns.heatmap(corr, annot=True, fmt=".1f", mask=mask, vmax=.3, center=0, square=True, linewidths=.5,
                cbar_kws={"shrink": .5})
